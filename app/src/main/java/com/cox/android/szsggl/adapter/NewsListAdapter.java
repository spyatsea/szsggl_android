...
public class NewsListAdapter extends BaseAdapter implements Filterable {
	...
	private final int TYPE_SMALL_PIC = 1;
	...
	public int getItemViewType(int position) {
		...
		Map<String, Object> info = (HashMap<String, Object>) data.get("info");
		Integer viewType = 1;
		String insInfoType = (String) info.get("V_INFO_TYPE");
		if (CommonParam.NEWS_INFO_TYPE_BIG_PIC.equals(insInfoType)) {
			// 大图信息
			viewType = TYPE_BIG_PIC;
		} else if (CommonParam.NEWS_INFO_TYPE_SMALL_PIC.equals(insInfoType)){
			// 小图信息
			viewType = TYPE_SMALL_PIC;
		} else {
			// 普通信息
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (currentType == TYPE_BIG_PIC) {
			// 大图信息
			mDropDownResource = R.layout.news_t1_list_big_image_item;
			view = createViewFromResource(position, convertView, parent, R.layout.news_t1_list_big_image_item);
			convertView = view;
		} else if (currentType == TYPE_SMALL_PIC) {
			// 小图信息
			mDropDownResource = R.layout.news_t1_list_small_image_item;
			view = createViewFromResource(position, convertView, parent, R.layout.news_t1_list_small_image_item);
			convertView = view;
		} else {
			// 普通信息
			mDropDownResource = R.layout.news_t1_list_normal_item;
			view = createViewFromResource(position, convertView, parent, R.layout.news_t1_list_normal_item);
			convertView = view;
...
//		}
	...
	public static interface ViewBinder {
		 ...
		 *            a safe String representation of the supplied data: it is either the result of data.toString() or
