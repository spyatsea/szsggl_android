...
import com.cox.android.szsggl.adapter.InsDkHistoryListAdapter;
...
public class InsDkHistoryListReverseLandActivity extends DbActivity {
    /**
     * 当前类对象
     * */
     ...
     * 列表Adapter
     */
    private InsDkHistoryListAdapter infoListAdapter;
    /**
     * 主进程 AsyncTask 对象
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        classThis = InsDkHistoryListReverseLandActivity.this;
        ...
        setContentView(R.layout.ins_dk_history_list);
    ...
    private class MainTask extends AsyncTask<Object, Integer, String> {
        ...
        protected void onProgressUpdate(Integer... progress) {
            if (progress[0] == PROGRESS_SET_FIELD) {
                ...
                // 生成信息列表
                infoListAdapter = (InsDkHistoryListAdapter) infoList.getAdapter();
                if (infoListAdapter == null) {
                    infoListAdapter = new InsDkHistoryListAdapter(getApplicationContext(), listItems, R.layout.ins_dk_history_list_item,
                    ...
                    // 对绑定的数据进行处理
                    infoListAdapter.setViewBinder(new InsDkHistoryListAdapter.ViewBinder() {
                        ...
                        public boolean setViewValue(View view, Object data, String textRepresentation) {
                            if (view instanceof Button) {
                                    ...
                                    btn.setOnClickListener(new OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            ...
                                            if (!baseApp.isReverseRotate) {
                                                intent = new Intent(classThis, InsDkShowLandActivity.class);
                                            } else {
                                                intent = new Intent(classThis, InsDkShowReverseLandActivity.class);
                                            }
                                            // 将数据存入 Intent 中
                                ...
                                if (textView.getId() == R.id.infoSn) {
                                    ...
                                    textView.setText(CommonUtil.N2B((String) info.get("ctime")));
                                    ...
                                    textView.setText(CommonUtil.N2B((String) info.get("jingwd")));
                                }
                                return true;
                            } else {
                                return false;
    ...
    private class SearchTask extends AsyncTask<Object, Integer, String> {
        ...
        protected String doInBackground(Object... params) {
            ...
            ArrayList<HashMap<String, Object>> recList = (ArrayList<HashMap<String, Object>>) infoTool
                    .getInfoMapList(
                            "select * from t_szfgs_sgxunsqdjl model where model.valid='1' and model.biz_id=? and model.res_id=? and model.quid=? order by model.ctime desc",
        ...
        protected void onPostExecute(String result) {
            ...
            if (CommonParam.RESULT_SUCCESS.equals(result)) {
                ...
                infoListAdapter.notifyDataSetChanged();
                titleText.setText("签到打卡记录(" + listItems.size() + ")");
            } else {
                show("数据加载失败");
            }
            isConnecting = false;
    ...
    public class DeleteInfoTask extends AsyncTask<Object, Integer, String> {
        ...
        protected String doInBackground(Object... params) {
            ...
            dataList = (ArrayList<HashMap<String, Object>>) infoTool
                    .getInfoMapList(
                            "select model.photo, model.video, model.audio from t_szfgs_sgxunsqdjl model where model.valid='1' and model.ids=? and model.quid=?",
            ...
            // 删除信息。开始=================================================================
            // ★☆
            long insResult = infoTool.delete("t_szfgs_sgxunsqdjl", "ids=? and quid=?", new String[]{(String) info.get("ids"), (String) baseApp.getLoginUser().get("ids")});
            if (insResult > -1L) {
                // 删除记录附件。开始=================================================================
