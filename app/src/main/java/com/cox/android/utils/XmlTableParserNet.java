...
public class XmlTableParserNet {
	...
	private Map<String, Object> readDataset(XmlPullParser parser) throws XmlPullParserException, IOException {
		...
		while (parser.next() != XmlPullParser.END_TAG) {
			...
			if (name.equals("Table")) {
				...
				// 数据表列定义
				columns = readSchema(parser, ns_xs, "schema");
			} else {
				skip(parser);
			}
		}
	 ...
	 * 读取数据表列定义的内容
	 * */
	private List<String> readSchema(XmlPullParser parser, String namespace, String tagName)
		...
		while (parser.next() != XmlPullParser.END_TAG) {
			...
			if (name.equals("sequence")) {
				columns = readColumns(parser, namespace, "sequence");
			}
		}
