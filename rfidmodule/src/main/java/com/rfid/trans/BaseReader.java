...
public class BaseReader {
		...
		try{
			while((System.currentTimeMillis()-beginTime)<endTime)
			{
				...
				if(buffer!=null)
				{
	                ...
	                {
	                	if(((daw[index]&255)>=4)&&(((daw[index+2]&255)==cmd) ||((daw[index+3]&255)==0xFE &&((daw[index+2]&255)==0x0)) ))
	                	{
	...
	public int SetPowerMode(byte ComAddr,byte OnOff)
	{
		...
		getCRC(buffer,buffer[0]-1);
		SendCMD(buffer);
		int result = GetCMDData(recvBuff,recvLength,0x6B,1000);
		if(result==0)
		{
			return recvBuff[3]&255;
		}
    ...
    public int SetBaudRate(byte ComAddr,byte baud)
    {
        ...
        getCRC(buffer,buffer[0]-1);
        SendCMD(buffer);
        int result = GetCMDData(recvBuff,recvLength,0x28,1000);
        if(result==0)
        {
            return recvBuff[3]&255;
        }
